...
const AdminDimensions = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const dikmensiResponse = await axios.get(`${baseUrl}dikmensi`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedDikmensiId) {
        response = await axios.put(`${baseUrl}admin/dikmensi/${selectedDikmensiId}`, formData);
      } else {
        response = await axios.post(`${baseUrl}admin/dikmensi`, formData);
      }
