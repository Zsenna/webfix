...
const AdminTeachers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const guruResponse = await axios.get(`${baseUrl}guru`);
        setGuru({
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      if (selectedGuruId) {
        const response = await axios.put(`${baseUrl}admin/guru/${selectedGuruId}`, formData);
        ...
        const response = await axios.post(`${baseUrl}admin/guru`, formData);
        if (response.data.Status === 'Success') {
    ...
    <main className="overflow-auto font-poppins">
      ...
      <div className="mx-auto my-12">
        <div className="mb-12 flex flex-col items-center justify-center text-center">
          <Box
            styles="w-48 h-48 mb-2 relative group hover:cursor-pointer overflow-hidden"
            onClick={() => {
               openEditModal(guru.data[0]);
            }}
          >
            <img src={`${baseUrl}${guru.data[0]?.fotoGuru}`} alt="" />
        ...
        <div className="mx-auto flex flex-col justify-items-center gap-x-6 gap-y-12 sm:grid sm:grid-cols-2 lg:grid-cols-3 lg:gap-x-0 lg:px-6 xl:grid-cols-4">
          {guru.data.slice(1).map((guru) => (
            ...
            >
              ...
              >
                <img src={`${baseUrl}${guru.fotoGuru}`} alt="" />
                <button
