...
const addAnnoucementPage = (
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${baseUrl}admin/pengumuman`, formData);
...
const AdminAnnouncement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const noticeResponse = await axios.get(`${baseUrl}pengumuman/:id`);
   ...
   const openEditModal = (notice) => {
    setjudul(notice.judul);
    setDescNotice(notice.descNotice);
    setDocument(notice.document);
    setdate(notice.date);
