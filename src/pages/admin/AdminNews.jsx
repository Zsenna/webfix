...
const addNewsPage = (
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${baseUrl}admin/berita`, formData);
...
const AdminAnnouncement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const newsResponse = await axios.get(`${baseUrl}berita/:id`);
  ...
  const openEditModal = (news) => {
    setJudulBerita(news.judulBerita);
    setIsiBerita(news.isiBerita);
    setDate(news.date);
    setSampul(news.sampul);
    setAddNews(true);
  };
