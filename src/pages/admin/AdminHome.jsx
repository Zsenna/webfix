...
const AdminHome = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${baseUrl}berita/:id`)
      .then(res => setNews(res.data))
      .catch(err => console.log(err));
    
    axios.get(`${baseUrl}pengumuman/:id`)
      .then(res => setNotice(res.data))
      .catch(err => console.log(err));
    
    axios.get(`${baseUrl}guru/`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedHomeId) {
        response = await axios.put(`${baseUrl}admin/beranda/${selectedHomeId}`, formData);
      } else {
        response = await axios.post(`${baseUrl}admin/beranda`, formData);
      }
              ...
              key={homeItem.ID}>
                <div className="grid h-full mb-5 grid-cols-1 justify-center gap-8 sm:grid-rows-[384px] md:grid-cols-[384px_1fr]">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div
                      className="bg-light-green h-96 overflow-auto rounded-b-xl px-8 py-4 text-justify leading-6 sm:h-full sm:rounded-tr-xl"
                      onClick={() => handleEditHome(homeItem)}
                      key={homeItem.ID}>
                      { home[0]?.description }
                    </div>
                      ...
                      {previewKImg ? (
                        ...
                        <div>
                          <input
                            ...
                            name="kImg"
                          />
                          {kImg && <img src={`${baseUrl}${home[0]?.kImg}`} alt="" />}
                        </div>
                      )}
                    </PhotoBox>
                  </label>
