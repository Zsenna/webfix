      ...
      <div className="mx-auto my-12 flex max-w-7xl flex-col gap-8 px-4 lg:px-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          <div className="flex items-center justify-center sm:block">
            <Box styles="w-52 h-52">
              <img src={`${baseUrl}${siswa.fotoMurid}`} alt={siswa.nama} />
            </Box>
          </div>

          <div className="grid grid-cols-2">
            <p>Nama Peserta Didik</p>
            <p className="before:mr-1 before:content-[':'] break-words">{siswa.nama}</p>

            <p>Kelas Peserta Didik</p>
            <p className="before:mr-1 before:content-[':'] break-words">{siswa.kelas}</p>

            <p>NIS Peserta Didik</p>
            <p className="before:mr-1 before:content-[':'] break-words">{siswa.nis}</p>

            <p>Alamat</p>
            <p className="leading-6 before:mr-1 before:content-[':'] break-words">
              {siswa.alamat}
            </p>

            <p>Status Peserta Didik</p>
            <p className="capitalize before:mr-1 before:content-[':'] break-words">
              {siswa.status}
            </p>
          </div>
        </div>
        ...
        <form className="flex flex-col gap-4">
          <p className="text-2xl font-medium">Unggah Dokumen</p>
            {/* <img src={`${baseUrl}${siswa.dokumen}`} alt={siswa.dokumen} /> */}
...
const LoginPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const siswwaResponse = await axios.get(`${baseUrl}siswa`);
  ...
  const downloadFile = async (fileName) => {
    try {
      const url = `${baseUrl}siswa/download/${fileName}`;
